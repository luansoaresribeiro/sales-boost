...
| B5 | **Dois sistemas diferentes de "tracking do Instagram" rodando em paralelo, sem se falar:** (1) `instagram-performance` — API oficial do Instagram, alimenta a aba Performance real (`instagram_content_performance`); (2) `marketing-ai`'s `runTracking()` — scraping via **Apify**, tabela separada (`marketing_ai_tracking_snapshots`), roda 1x/dia no cron `marketing-ai-daily`, e é o que alimenta `marketing_ai_insights` (pillar `'tracking'`) que por sua vez informa `creative-ideas`/`creative-generate`. Os dois podem mostrar números diferentes pro mesmo Instagram. Descoberto em 2026-09-11 ao rastrear se a geração de imagem usa o Agente de Dados — confirmado que só usa via este segundo caminho (indireto, defasado até 1 dia), nunca lê `instagram_content_performance` diretamente. **Adiado a pedido do dono — resolver numa próxima rodada.** | `supabase/functions/marketing-ai/index.ts` (`runTracking`) vs `supabase/functions/instagram-performance/index.ts` | Números de Instagram inconsistentes entre abas; decisões de conteúdo baseadas em dado até 1 dia desatualizado em vez do real mais recente |
